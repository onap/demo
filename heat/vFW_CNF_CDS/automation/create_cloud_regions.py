...
def add_availability_zone(cloud_region):
    ...
    cloud_type = Config.CLOUD_REGIONS[region_id]["cloud_type"]
    try:
        az = cloud_region.get_availability_zone_by_name(availability_zone_name)
...
def update_connectivity_info(region_id):
    ...
    try:
        ...
        connectivity_info = ConnectivityInfo.create(cloud_region_id=region_id,
...
def add_region_to_so_db(region):
    ...
    if is_k8s_region(region.cloud_region_id):
        ...
        result = SoDbAdapter.add_cloud_site(cloud_region_id=region.cloud_region_id,
