...
open class K8sHealthCheck : AbstractScriptComponentFunction() {
    ...
    override suspend fun processNB(executionRequest: ExecutionServiceInput) {
        ...
        val heatStackIdStatusMap = hashMapOf<String, Boolean>()

        configValueSetup.fields().forEach { it ->
            val instanceName = it.value.get("k8s-instance-id").asText()
            heatStackIdStatusMap[instanceName] = false
            ...
            if (resourcesStatus != null) {
                for (status in resourcesStatus) {
                    if (instanceStatusIsOk(status)) {
                        heatStackIdStatusMap[instanceName] = true
                    } else {
                        // TODO: For testing
                        log.info("instance status false")
