...
open class K8sHealthCheck : AbstractScriptComponentFunction() {
    ...
    override suspend fun processNB(executionRequest: ExecutionServiceInput) {
        ...
        log.info("Module AAI: $modulesAai")

        val heatStackIdStatusMap: Map<String, Boolean>
        for (aaiModule in modulesAai) {
            val heatStackId: Any = aaiModule["heat-stack-id"].asText()
            heatStackIdStatusMap[heatStackId] = false
        }
        ...
        for (instance in heatStackIdStatusMap.keys) {
            val instanceStatus: K8sRbInstanceStatus = instanceApi.getInstanceStatus(instance).asText())!!
            ...
            if (resourcesStatus != null) {
                for (status in resourcesStatus) {
                    if (instanceStatusIsOk(status)) {
                        heatStackIdStatusMap[instance] = true
                    } else {
                        // TODO: For testing
        ...
        log.info("Instances status: $heatStackIdStatusMap")
