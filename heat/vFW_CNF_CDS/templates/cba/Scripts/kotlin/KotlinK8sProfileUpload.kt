...
open class K8sProfileUpload : AbstractScriptComponentFunction() {
        ...
        for (prefix in prefixList) {
            ...
            if (k8sRbProfileName.equals("")) {
                ...
                if (api.hasProfile(k8sRbProfileName)) {
                    ...
                    var profileFileName: Path = Paths.get(bluePrintBasePath.plus(File.separator).plus("Templates").plus(File.separator).plus("k8s-profiles").plus(File.separator).plus("${k8sRbProfileName}.tar.gz"))
                    log.info("Reading K8s profile file: ${profileFileName}")

                    profileFileName = prepareProfileFile(profileFileName)
                    ...
                    api.uploadProfileContent(profile, profileFileName)
    ...
    fun prepareProfileFile(profileFileName: Path): Path {
        val profileFile = profileFileName.toFile()

        if (!profileFile.exists())
            throw BluePrintProcessorException("K8s Profile template file ${profileFileName} does not exists")
