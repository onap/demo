...
def upgrade_vsp(config):
    ...
    if vnfs:
        for vnf in vnfs:
            ...
            #mypath = os.path.dirname(os.path.realpath(__file__))
            #vsp_path = 
            #logger.info (" vsp PATH=", vsp_path)
            ##Above logic to be use once MakeFile changes are available
            logger.info("******** Upgrade vsp - {vsp_name} *******",vsp_name)
            vsp = Vsp(vsp_name)
            vsp.create_new_version()
            vsp.update_package(open("vsp/native_cnf_k8s_demo1.zip", 'rb'))
            vsp.onboard()
            ...
            ##This return need to shift after demo a tab
    return vf
...
def main():
    ...
    if choice == 1:
            ...
            service.add_resource(new_vnf)
            set_properties_for_upgrade(service, xnf=new_vnf)
