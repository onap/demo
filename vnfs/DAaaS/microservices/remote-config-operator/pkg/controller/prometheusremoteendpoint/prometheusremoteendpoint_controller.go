...
func (r *ReconcilePrometheusRemoteEndpoint) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if isBeingDeleted {
		if instance.Spec.Type == "m3db" {
			//Delete Remote write
			if err := r.processM3dbDeletionRequest(reqLogger, instance); err != nil {
				reqLogger.Error(err, "Error processing M3DB deletion request")
				return reconcile.Result{}, err
			}
		} else {
...
func (r *ReconcilePrometheusRemoteEndpoint) processM3dbRequest(reqLogger logr.Logger, instance *onapv1alpha1.PrometheusRemoteEndpoint) error {
	...
	if len(specs) == 0 {
		reqLogger.Info("No Remote write exist, creating the first one...")
...
func (r *ReconcilePrometheusRemoteEndpoint) processM3dbDeletionRequest(reqLogger logr.Logger, instance *onapv1alpha1.PrometheusRemoteEndpoint) error {
	prom := &monitoringv1.Prometheus{}
	if err1 := r.client.Get(context.TODO(), types.NamespacedName{Namespace: "edge1", Name: "cp-prometheus-prometheus"}, prom); err1 != nil {
		reqLogger.Error(err1, "Error getting prometheus")
		return err1
