...
func (r *ReconcileGrafanaDataSource) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	for _, datasource := range datasources {
		...
		reqLogger.Info("datasource URL in the GET response body", "URL", respURL)
		...
		if getResp.StatusCode == http.StatusNotFound {
			...
			// create datasource
			createDataSource(grafana, datasource)
			...
			if respURL != datasource.URL || respIsDefault.(bool) != datasource.IsDefault || respAccess != datasource.Access {
				updateDatasource(grafana, datasource, respID)
			} else {
...
func createDataSource(grafana map[string]string, datasource onapv1alpha1.Datasource) {
...
func updateDatasource(grafana map[string]string, datasource onapv1alpha1.Datasource, datasourceID string) {
	reqLogger := log.WithValues("Datasource name", datasource.Name, "Datasource URL", datasource.URL)
