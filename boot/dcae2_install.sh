...
wget -P /opt/app/inputs-templates https://nexus.onap.org/service/local/repositories/raw/content/org.onap.dcaegen2.deployments/releases/scripts/designate_input_onap.yaml



# generate blueprint input files
pip install jinja2
wget https://nexus.onap.org/service/local/repositories/raw/content/org.onap.dcaegen2.deployments/releases/scripts/detemplate-bpinputs.py && \
  (python detemplate-bpinputs.py /opt/config /opt/app/inputs-templates /opt/app/config; \
   rm detemplate-bpinputs.py)

# posting ONAP DNS records to Designate
AUTHURL=$(cat /opt/config/keystone_url.txt)
...
wget https://nexus.onap.org/service/local/repositories/raw/content/org.onap.dcaegen2.deployments/releases/scripts/designate_import.py && \
  (python designate_import.py /opt/app/inputs-templates/designate_input_onap.yaml \
                              $AUTHURL $USERNAME $PASSWORD $TENANTNAME; \
   rm detemplate-bpinputs.py)
